...
export class DeckGLMap {
  ...
  private merchantCargoVessels: MerchantCargoVessel[] = [];
  ...
  private buildLayers(): LayersList {
    ...
    // Live merchant cargo / tanker AIS (AISStream relay)
    if (mapLayers.cargoShips && this.merchantCargoVessels.length > 0) {
      layers.push(this.createMerchantCargoTrailsLayer());
      layers.push(this.createMerchantCargoLayer());
    }
  ...
  private createMerchantCargoLayer(): IconLayer {
    // White SVG triangle; tint via getColor.
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64">
      <path d="M32 6 L56 56 L32 46 L8 56 Z" fill="#ffffff" stroke="rgba(0,0,0,0.35)" stroke-width="2" />
      <path d="M32 10 L50 52 L32 44 L14 52 Z" fill="rgba(255,255,255,0.85)" opacity="0.35"/>
    </svg>`;
    const url = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`;

    return new IconLayer({
      id: 'merchant-cargo-layer',
      data: this.merchantCargoVessels,
      pickable: true,
      getPosition: (d) => [d.lon, d.lat],
      getIcon: () => ({
        url,
        width: 64,
        height: 64,
        // Anchor tip near center; triangle points "up" in SVG.
        anchorX: 32,
        anchorY: 34,
      }),
      getColor: (d) => {
        const st = (d.shipTypeLabel || '').toLowerCase();
        if (st.includes('tanker')) return [80, 200, 255, 235] as [number, number, number, number];
        return [60, 220, 140, 235] as [number, number, number, number];
      },
      getAngle: (d) => {
        const hdg = Number.isFinite(d.heading) && d.heading > 0 ? d.heading : d.cog;
        // deck.gl rotates clockwise degrees; align "up" triangle to heading.
        return hdg || 0;
      },
      // Size in pixels (not meters); MarineTraffic-like small arrows.
      sizeUnits: 'pixels',
      getSize: (d) => ((d.shipTypeLabel || '').toLowerCase().includes('tanker') ? 16 : 14),
      sizeMinPixels: 10,
      sizeMaxPixels: 26,
      billboard: true,
      alphaCutoff: 0.05,
  ...
  private createMerchantCargoTrailsLayer(): PathLayer {
    const data = (this.merchantCargoVessels ?? []).filter(v => (v.track?.length ?? 0) >= 2);
    return new PathLayer({
      id: 'merchant-cargo-trails-layer',
      data,
      getPath: (d) => d.track as unknown as number[][],
      getColor: (d) => {
        const st = (d.shipTypeLabel || '').toLowerCase();
        if (st.includes('tanker')) return [80, 200, 255, 140] as [number, number, number, number];
        return [60, 220, 140, 140] as [number, number, number, number];
      },
      widthUnits: 'pixels',
      getWidth: 2,
      widthMinPixels: 1,
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'merchant-cargo-layer':
        return {
          html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.shipTypeLabel || 'AIS')} · ${Number(obj.sog ?? 0).toFixed(1)} kn · ${Math.round(Number(obj.cog ?? 0))}°<br/><span style="opacity:.75">MMSI ${text(obj.mmsi)}</span></div>`,
        };
  ...
  private handleClick(info: PickingInfo): void {
    ...
    const layerToPopupType: Record<string, PopupType> = {
      ...
      'merchant-cargo-layer': 'civilianVessel',
  ...
  public setMerchantCargoVessels(vessels: MerchantCargoVessel[]): void {
    this.merchantCargoVessels = vessels;
    this.render();
  }
