...
import type { MapLayers, Hotspot, MilitaryFlight, MilitaryVessel, MilitaryVesselCluster, MerchantCargoVessel, NaturalEvent, InternetOutage, CyberThreat, SocialUnrestEvent, UcdpGeoEvent, MilitaryBase, GammaIrradiator, Spaceport, EconomicCenter, StrategicWaterway, CriticalMineralProject, AIDataCenter, UnderseaCable, Pipeline, CableAdvisory, RepairShip, AisDisruptionEvent, AisDensityZone, AisDisruptionType } from '@/types';
import type { Earthquake } from '@/services/earthquakes';
import type { AirportDelayAlert } from '@/services/aviation';
import { MapPopup } from './MapPopup.ts';
import type { MapContainerState, MapView, TimeRange } from './MapContainer.ts';
import type { CountryClickPayload } from './DeckGLMap.ts';
...
interface MerchantCargoMarker extends BaseMarker {
  _kind: 'merchantCargo';
  mmsi: string;
  name: string;
...
interface GlobePath {
  ...
  name: string;
  points: number[][];
  pathType: 'cable' | 'oil' | 'gas' | 'products' | 'orbit' | 'stormTrack' | 'stormHistory' | 'cargoTrail';
  status: string;
  country?: string;
  windKt?: number;
}
...
type GlobeMarker =
  ...
  | WebcamMarkerData | WebcamClusterData | MerchantCargoMarker;
...
export class GlobeMap {
  ...
  private merchantCargoMarkers: MerchantCargoMarker[] = [];
  private merchantCargoData = new Map<string, MerchantCargoVessel>();
  private satelliteMarkers: SatelliteMarker[] = [];
  private satelliteTrailPaths: GlobePath[] = [];
  private stormTrackPaths: GlobePath[] = [];
  private cargoTrailPaths: GlobePath[] = [];
  ...
  private async initGlobe(): Promise<void> {
    ...
    (globe as any)
      ...
      .pathColor((d: GlobePath) => {
        ...
        if (d.pathType === 'cargoTrail') {
          const s = (d.status || '').toLowerCase();
          if (s.includes('tanker')) return 'rgba(80,200,255,0.55)';
          return 'rgba(60,220,140,0.55)';
        }
        if (d.pathType === 'cable') {
      ...
      .pathStroke((d: GlobePath) => {
        ...
        if (d.pathType === 'cargoTrail') return 0.9;
      ...
      .pathDashLength((d: GlobePath) => {
        ...
        if (d.pathType === 'cargoTrail') return 0.6;
      ...
      .pathDashGap((d: GlobePath) => {
        ...
        if (d.pathType === 'cargoTrail') return 0.25;
      ...
      .pathDashAnimateTime((d: GlobePath) => {
        ...
        if (d.pathType === 'cable') return 0;
        if (d.pathType === 'cargoTrail') return 4000;
  ...
  private buildMarkerElement(d: GlobeMarker): HTMLElement {
    ...
    if (d._kind === 'conflict') {
      ...
      el.innerHTML = GlobeMap.wrapHit(`<div style="font-size:11px;color:${sc};text-shadow:0 0 4px ${sc}88;">⛴</div>`);
      el.title = d.name;
    } else if (d._kind === 'merchantCargo') {
      const tc = (d.shipTypeLabel || '').toLowerCase().includes('tanker') ? '#50c8ff' : '#3cdc8a';
      const angle = Number.isFinite(d.cog) ? d.cog : 0;
      // Triangle/arrow marker oriented by course over ground (MarineTraffic-like).
      el.innerHTML = GlobeMap.wrapHit(`
        <div style="
          width:12px;height:12px;
          background:${tc};
          clip-path:polygon(50% 0%, 100% 85%, 50% 65%, 0% 85%);
          transform:rotate(${angle}deg);
          box-shadow:0 0 6px ${tc}66;
          border:1px solid rgba(0,0,0,0.25);
        "></div>
      `);
      el.title = `${d.name} · ${d.shipTypeLabel || 'AIS'} · ${Math.round(d.cog)}°`;
  ...
  private handleMarkerClick(d: GlobeMarker, anchor: HTMLElement): void {
    ...
    if (d._kind === 'merchantCargo' && this.popup) {
      const vessel = this.merchantCargoData.get(d.mmsi);
      if (vessel) {
        const aRect = anchor.getBoundingClientRect();
        const cRect = this.container.getBoundingClientRect();
        const x = aRect.left - cRect.left + aRect.width / 2;
        const y = aRect.top - cRect.top;
        this.hideTooltip();
        this.popup.show({ type: 'civilianVessel', data: vessel, x, y });
        return;
      }
    }
  ...
  private flushMarkersImmediate(): void {
    ...
    if (this.layers.cargoShips) markers.push(...this.merchantCargoMarkers);
  ...
  private flushPaths(): void {
    ...
    const cargoPaths = this.layers.cargoShips ? this.cargoTrailPaths : [];
    (this.globe as any).pathsData([...paths, ...orbitPaths, ...stormPaths, ...cargoPaths]);
  }
  ...
  public setMerchantCargoVessels(vessels: MerchantCargoVessel[]): void {
    this.merchantCargoData.clear();
    for (const v of vessels ?? []) this.merchantCargoData.set(v.mmsi, v);
    this.merchantCargoMarkers = (vessels ?? []).map(v => ({
      _kind: 'merchantCargo' as const,
      _lat: v.lat,
      _lng: v.lon,
      mmsi: v.mmsi,
      name: v.name,
    ...
    this.cargoTrailPaths = (vessels ?? [])
      .filter(v => (v.track?.length ?? 0) >= 2)
      .map(v => ({
        id: `cargo-${v.mmsi}`,
        name: v.name || `MMSI ${v.mmsi}`,
        points: (v.track ?? []) as unknown as number[][],
        pathType: 'cargoTrail' as const,
        status: v.shipTypeLabel ?? '',
      }));
    this.flushMarkers();
    this.flushPaths();
  }
