...
export class MapComponent {
  ...
  private createLayerToggles(): HTMLElement {
    ...
    const fullLayers: (keyof MapLayers)[] = [
      ...
      'ais', 'cargoShips', 'flights', 'gpsJamming',                      // transport/interference
    ...
    const layerLabelKeys: Partial<Record<keyof MapLayers, string>> = {
      ...
      cargoShips: 'components.deckgl.layers.liveCargoAis',
  ...
  private renderOverlays(projection: d3.GeoProjection): void {
    ...
    // Live merchant cargo / tanker AIS (same feed as DeckGL; dev relay)
    if (this.state.layers.cargoShips) {
      this.merchantCargoVessels.forEach((vessel) => {
        const pos = projection([vessel.lon, vessel.lat]);
        if (!pos) return;

        const div = document.createElement('div');
        div.className = 'merchant-cargo-marker';
        div.style.left = `${pos[0]}px`;
        div.style.top = `${pos[1]}px`;

        const dot = document.createElement('div');
        dot.className = 'merchant-cargo-dot';
        const tanker = (vessel.shipTypeLabel || '').toLowerCase().includes('tanker');
        dot.style.backgroundColor = tanker ? '#50c8ff' : '#3cdc8a';
        div.appendChild(dot);

        if (this.state.zoom >= 3) {
          const label = document.createElement('div');
          label.className = 'merchant-cargo-label';
          label.textContent = vessel.name.length > 18 ? vessel.name.slice(0, 18) + '…' : vessel.name;
          div.appendChild(label);
        }
