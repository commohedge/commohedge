import type { ConflictZone, Hotspot, NewsItem, MilitaryBase, StrategicWaterway, APTGroup, NuclearFacility, EconomicCenter, GammaIrradiator, Pipeline, UnderseaCable, CableAdvisory, RepairShip, InternetOutage, AIDataCenter, AisDisruptionEvent, SocialUnrestEvent, MilitaryFlight, MilitaryVessel, MilitaryFlightCluster, MilitaryVesselCluster, MerchantCargoVessel, NaturalEvent, Port, Spaceport, CriticalMineralProject, CyberThreat } from '@/types';
...
export class MapPopup {
  ...
  private renderContent(data: PopupData): string {
    switch (data.type) {
      ...
      case 'civilianVessel':
        return this.renderMerchantCargoPopup(data.data as MerchantCargoVessel);
...
${isFeatureAvailable('wingbitsEnrichment') ? '<div class="wingbits-live-section"><div class="wingbits-live-loading" style="font-size:11px;opacity:0.5;padding:4px 0">Loading Wingbits live data…</div></div>' : ''}
  ...
  private renderMerchantCargoPopup(vessel: MerchantCargoVessel): string {
    const name = escapeHtml(vessel.name || `MMSI ${vessel.mmsi}`);
    const type = escapeHtml(vessel.shipTypeLabel || 'AIS');
    const mmsi = escapeHtml(vessel.mmsi);
    const dest = vessel.destination ? escapeHtml(vessel.destination) : '—';
    const when = vessel.updatedAt.toLocaleString();
    return `
      <div class="popup-header">
        <span class="popup-title">&#128741; ${name}</span>
        <span class="popup-badge" style="background:rgba(60,220,140,0.15);border:1px solid rgba(60,220,140,0.5);color:#3cdc8a;">${type}</span>
        <button class="popup-close" aria-label="Close">×</button>
      </div>
      <div class="popup-body">
        <div class="popup-stats">
          ...
          <div class="popup-stat">
            ...
            <span class="stat-value">${vessel.sog.toFixed(1)} kn</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">COG</span>
            <span class="stat-value">${Math.round(vessel.cog)}°</span>
          </div>
          <div class="popup-stat">
        ...
        <p class="popup-description" style="opacity:.85">Destination: ${dest}</p>
        <p class="popup-description" style="opacity:.65;font-size:0.8em">Last update: ${when}<br/>
        Data: AIS (AISStream community feed)</p>
      </div>`;
  }
