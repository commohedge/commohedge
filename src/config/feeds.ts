...
/** RSS sources for the commodity variant — exported for the Commodity News page (all builds). */
export const COMMODITY_FEEDS: Record<string, Feed[]> = {
  ...
  // Aligné sur Latest market news (Fertilizers + feedgrade minerals) — pas de RSS Argus public ; Google News site: + filtre host dans CommodityNews.
  'fertilizer-news': [
    {
      name: 'Argus — fertilizer & feedgrade minerals',
      url: rss(
        'https://news.google.com/rss/search?q=site:argusmedia.com+(fertilizer+OR+fertiliser+OR+feedgrade+OR+"feed+grade"+OR+urea+OR+potash+OR+phosphate+OR+ammonia+OR+nitrogen+OR+DAP+OR+MOP+OR+NPK+OR+sulphur+OR+"phosphoric+acid")+when:90d&hl=en-US&gl=US&ceid=US:en',
      ),
    },
    {
      name: 'Argus — ammonia & nutrient news',
      url: rss(
        'https://news.google.com/rss/search?q=site:argusmedia.com+(ammonia+OR+"ammonium+nitrate"+OR+MOP+OR+DAP+OR+MAP+OR+UAN+OR+SSP)+when:90d&hl=en-US&gl=US&ceid=US:en',
      ),
    },
  ],
  markets: [
