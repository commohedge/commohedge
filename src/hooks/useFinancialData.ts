...
export const useFinancialData = (): UseFinancialDataReturn => {
  ...
  const autoGenerateExposures = useCallback(() => {
    try {
      ...
      const exposuresBeforeClear = serviceRef.current.getExposures().length;
      serviceRef.current.clearAutoGeneratedExposures();
      const exposuresAfterClear = serviceRef.current.getExposures().length;
      console.log(`[FX EXPOSURE] Cleared ${exposuresBeforeClear - exposuresAfterClear} old auto-generated exposures`);
