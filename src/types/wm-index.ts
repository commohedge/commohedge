...
export interface MapLayers {
  ...
  hotspots: boolean;
  ais: boolean;
  /** Live merchant cargo / tanker positions (AIS feed, e.g. AISStream via dev relay). */
  cargoShips: boolean;
  nuclear: boolean;
  irradiators: boolean;
...
/** Civilian merchant vessel from an AIS position feed (distinct from military tracking). */
export interface MerchantCargoVessel {
  id: string;
  mmsi: string;
  name: string;
  ...
  cog: number;
  /** True heading if known (degrees). */
  heading: number;
  /** Decoded AIS ship type label when known. */
