...
const COMMODITIES_SYSTEM_PROMPT = `Tu es le Hedge Assistant de la plateforme Commodity Risk Manager, expert en Risk Management (matières premières et change). Tu aides les utilisateurs à comprendre et gérer leurs risques sur les matières premières et le change. Stratégies: forwards, futures, swaps, options. Analyse: exposition prix, risque de base. Marchés: LME, NYMEX/ICE, CBOT/CME. Ne te présente pas longuement en début de réponse.`;
...
function buildSystemPrompt(settings: ChatSettings, marketDataContext: string | null, appCommodityContext?: AppCommodityContext | null): string {
  ...
  const identity = "Tu es le Hedge Assistant de la plateforme Commodity Risk Manager (hedging matières premières et change).";
  let prompt = isForex
    ? `${identity} Tu traites le hedging change (FOREX) et matières premières. Stratégies: forwards, options, swaps. Analyse des risques et réglementation IFRS 9. Ne te présente pas longuement en début de réponse.\n${RESPONSE_STYLE_INSTRUCTIONS[settings.responseStyle]}`
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller) {
      while (true) {
        ...
        if (done) {
          // Flush any remaining events (last chunk may not end with \n\n)
